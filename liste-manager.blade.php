        ...
        <div class="card">
            <div class="card-header text-center">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <h4 class="primary">Liste Manager</h4>
                    </div>
                    <div class="col-md-4">
                ...
                <table  class="table table-striped table-hover">
                    ...
                    <tbody>
                        @forelse ($liste_manager as $itemManager)
                            <tr>
                                <td><p class="tableau">{{ ($liste_manager->perPage() * ($liste_manager->currentPage() - 1 ))+ $loop->iteration }}</p></td>
                                <td><p class="tableau">{{ $itemManager->name }}</p></td>
                                <td><p class="tableau"> {{ $itemManager->email }}</p></td>
                                <td><p class="tableau"> {{ Str::upper($itemManager->roles->intitule) }}</p></td>
                                <td>
                                    <a class="btn btn-primary" href="{{ route('affichage-modification-manager', $itemManager) }}"><i class="bi bi-pencil"></i></a>&nbsp;
                                    <a class="btn btn-danger" href="{{ route('supprimer-manager', $itemManager) }}"><i class="bi bi-trash"></i></a>
                                </td>
                            </tr>
                        @empty
